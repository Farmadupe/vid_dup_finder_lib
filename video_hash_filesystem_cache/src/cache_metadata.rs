...
impl FromStr for DecodeBackend {
    ...
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        match s.trim().to_lowercase().as_str() {
            "ffmpegbackend" => Ok(DecodeBackend::FfmpegBackend),
            "gstreamerbackend" => Ok(DecodeBackend::GstreamerBackend),
...
impl VdfCacheMetadata {
    pub fn new(crop: CropdetectType, skip_forward_amount: f64) -> Self {
    ...
    pub fn try_parse(val: &str) -> Result<VdfCacheMetadata, String> {
        ...
        match split[..] {
            [operating_system, decode_backend, crop, skip_forward_amount, cache_version] => {
                ...
                let crop = CropdetectType::from_str(crop)
