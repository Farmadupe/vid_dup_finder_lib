...
pub fn grid_images_rgb(images: &[&[RgbImage]]) -> Option<RgbImage> {
    ...
    let grid_num_x = images.iter().map(|i| i.len()).max()? as u32;
    ...
    Pixel: image::Pixel<Subpixel = Subpix>,
{
    ...
    let (img_x, img_y) = images.peek().map(|x: &&View| x.dimensions())?;
    let len = images.len() as u32;
    ...
    Pixel: image::Pixel<Subpixel = Subpix>,
{
    ...
    for (row_no, img) in images.enumerate() {
        let y_coord = row_no as u32 * img_y;
