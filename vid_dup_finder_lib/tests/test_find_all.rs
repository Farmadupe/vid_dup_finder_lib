...
impl HashesWithDistance {
    ...
    pub fn new(distance_from_start: u32, num_hashes: u32, rng: &mut StdRng) -> Self {
        let start_hash = VideoHash::random_hash(rng);
    ...
    pub fn with_start_hash(
        start_hash: VideoHash,
        ...
        for pair in members.iter().permutations(2) {
            let hash_1 = pair[0];
            let hash_2 = pair[1];
            let distance = hash_1.hamming_distance(hash_2);
...
impl HashesWithDistanceSet {
    pub fn new(
        ...
        let groups = (0..num_groups)
            .map(|_group_no| {
                let group_start_hash =
                    start_hash.hash_with_spatial_distance(current_group_distance, rng);
                current_group_distance += intergroup_distance;
                let group_size = hashes_per_group;
                hashes_per_group += 10;
                HashesWithDistance::with_start_hash(
                    group_start_hash,
                    intragroup_distance,
                    group_size,
                    rng,
...
fn test_find_dups_finds_a_known_group() {
    ...
    let groups = HashesWithDistanceSet::new(
        1,
        group_size,
        intergroup_distance,
        intragroup_distance,
    ...
    let dups = search(
        members,
        (intragroup_distance * 2) as f64 / TOLERANCE_SCALING_FACTOR,
    );
...
fn test_find_with_refs() {
    ...
    let dups = search_with_references(
        [start_hash.clone()],
        cand_hashes.clone(),
        intragroup_distance as f64 / TOLERANCE_SCALING_FACTOR,
    );
    ...
    let dups2 = search_with_references(
        start_hashes,
        cand_hashes,
        intragroup_distance as f64 / TOLERANCE_SCALING_FACTOR,
    );
