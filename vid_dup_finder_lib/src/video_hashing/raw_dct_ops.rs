...
pub fn idct_3d(matrix: &Array3<f64>) -> Array3<f64> {
    let mut matrix = matrix.clone();
    //first check that the supplied matrix is cube
    let (x_len, y_len, z_len) = matrix.dim();
    assert!({ x_len == y_len && x_len == z_len });
...
//normalize the values returned by idct operation, assuming that
//no normalization was done in the original DCT process.
// derived using https://www.fftw.org/fftw3_doc/1d-Real_002deven-DFTs-_0028DCTs_0029.html
#[cfg(feature = "app_only_fns")]
fn normalize_idct_3d(matrix: &mut Array3<f64>) {
    let (x_len, y_len, z_len) = matrix.dim();
    assert!({ x_len == y_len && x_len == z_len });

    let scaling_factor = ((x_len / 2).pow(3)) as f64;
