...
// can never be zero)
unsafe impl Send for GrayFrame {}
unsafe impl Sync for GrayFrame {}
...
impl VideoFrameInternal for GrayFrame {
    fn new(sample: gstreamer::Sample) -> Self {
        ...
        let data = frame
            .plane_data(0)
            .expect("Gray frame known to have a single plane");
...
impl ImageFns for GrayFrame {
    ...
    fn as_flat(&self) -> image::FlatSamples<&[u8]> {
        ...
        let data_ref: &[u8] = unsafe { std::slice::from_raw_parts(self.data_ptr, self.data_len) };
...
unsafe impl Send for RgbFrame {}
unsafe impl Sync for RgbFrame {}
...
impl VideoFrameInternal for RgbFrame {
    fn new(sample: gstreamer::Sample) -> Self {
        ...
        let data = frame
            .plane_data(0)
            .expect("Rgb frame known to have a single plane");
