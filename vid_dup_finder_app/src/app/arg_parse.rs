...
const MATCH_DB_RAW_PATHS: &str = "Raw match db paths";
const MATCH_DB_VALIDATE_ENTRIES_EXIST: &str = "Check all matchdb entries still exist";
const MATCH_DB_REMOVE_KNOWN_MATCHES: &str = "Return matches already in the database";
const MATCH_DB_REMOVE_UNKNOWN_MATCHES: &str = "Return matches not known in the database";
...
const DISPLAY_ORDERING: [&str; 29] = [
    ...
    MATCH_DB_PATH,
    MATCH_DB_RAW_PATHS,
    MATCH_DB_VALIDATE_ENTRIES_EXIST,
    MATCH_DB_REMOVE_KNOWN_MATCHES,
    MATCH_DB_REMOVE_UNKNOWN_MATCHES,
...
fn build_app() -> clap::Command {
    ...
    clap_app = clap_app.arg(
        clap::Arg::new(EXCL_EXTS)
            ...
            .value_delimiter(',')
            .action(Append)
            .default_value("png,jpg,bmp,jpeg,txt,text,db,gif,rb,py,mp3,wma,wav,ogg,db,flac,zip,rar,7z,pdf,htm,html,xls,doc,ppt,odt,ods,docx,xlsx,rtf,log,trashinfo,js,css,py,rs,aac,txt~,sh,DS_Store,kdenlive")
    ...
    clap_app = clap_app.arg(
        clap::Arg::new(MATCH_DB_RAW_PATHS)
            .long("matchdb-raw-paths")
            .num_args(0..)
            .help("raw match db paths")
            .value_parser(value_parser!(PathBuf))
            .display_order(get_ordering(MATCH_DB_RAW_PATHS)),
    );

    clap_app = clap_app.arg(
        clap::Arg::new(MATCH_DB_VALIDATE_ENTRIES_EXIST)
            .long("matchdb-validate-entries-exist")
            .requires(MATCH_DB_PATH)
            .action(SetTrue)
            .num_args(0)
            .display_order(get_ordering(MATCH_DB_VALIDATE_ENTRIES_EXIST)),
    );
    ...
    clap_app = clap_app.arg(
        clap::Arg::new(MATCH_DB_REMOVE_UNKNOWN_MATCHES)
            .long("matchdb-remove-unknown-matches")
            .requires(MATCH_DB_PATH)
            .conflicts_with_all([
                DISPLAY_MATCH_DB_MATCHES,
                DISPLAY_MATCH_DB_FALSEPOS,
            ...
            .action(SetTrue)
            .num_args(0)
            .display_order(get_ordering(MATCH_DB_REMOVE_UNKNOWN_MATCHES)),
    );
    clap_app = clap_app.arg(
...
pub fn parse_args() -> Result<AppCfg, AppError> {
    ...
    let hash_cfg = HashCfg {
        cropdetect: match args.get_one::<CropdetectTypeArg>(CROPDETECT) {
            None | Some(CropdetectTypeArg::None) => CropdetectType::NoCrop,
            Some(CropdetectTypeArg::Letterbox) => CropdetectType::Letterbox,
        },
        skip_forward: *args
            .get_one::<f64>(SKIP_FORWARD)
            .unwrap_or(&DEFAULT_HASH_CREATION_OPTIONS.skip_forward_amount),
    };
    ...
    let output_cfg = {
        ...
        let gui_available = cfg!(all(target_family = "unix", feature = "gui"));
        let gui_cfg = if gui_available && args.get_flag(GUI) {
            GuiOutputCfg::Gui {
                sorting,
    ...
    let matchdb_cfg = MatchDbCfg {
        ...
        raw_data_paths: args
            .get_many::<PathBuf>(MATCH_DB_RAW_PATHS)
            .map_or_else(Vec::new, |vals| vals.map(PathBuf::from).collect()),

        validate_entries_exist: args.get_flag(MATCH_DB_VALIDATE_ENTRIES_EXIST),
        remove_known_matches: args.get_flag(MATCH_DB_REMOVE_KNOWN_MATCHES),
        remove_unknown_matches: args.get_flag(MATCH_DB_REMOVE_UNKNOWN_MATCHES),
...
fn get_args_from_cmdline_or_file() -> Result<clap::ArgMatches, AppError> {
    let cmdline_args = build_app().get_matches();

    match cmdline_args.get_one::<PathBuf>(ARGS_FILE) {
        None => Ok(cmdline_args),
        Some(args_path) => get_argsfile_args(args_path),
    }
}

fn get_argsfile_args(argsfile_path: &Path) -> Result<clap::ArgMatches, AppError> {
